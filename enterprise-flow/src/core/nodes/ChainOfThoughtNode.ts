...
export class ChainOfThoughtNode implements Node {
  ...
  async compute(): Promise<Record<string, any>> {
    ...
    try {
      ...
      return {
        ...
        confidence: 0,
        alternatives: [],
        error: error.message || 'Chain of thought reasoning failed'
      };
    }
  }
   ...
   */
  private generateReasoningForStep(prompt: string, context: string, stepName: string, strategy: string): string {
   ...
   */
  private generateAlternatives(prompt: string, strategy: string): string[] {
  ...
  private describeCritique(strategy: string): string {
    ...
    return critiques[strategy] || critiques.analytical;
  }
  ...
  private describeLimitations(strategy: string): string {
    ...
    return limitations[strategy] || limitations.analytical;
  }
}
...
NodeRegistry.getInstance().registerNode({
  ...
  inputs: [
    { name: 'prompt', type: 'string', description: 'The primary question or problem to reason about' },
    { name: 'context', type: 'string', description: 'Additional context for reasoning', default: '' },
    { name: 'reasoningSteps', type: 'number', description: 'Number of reasoning steps to perform', default: 3, min: 1, max: 10 },
    { 
    ...
    { name: 'maxTokens', type: 'number', description: 'Maximum length of output', default: 500 },
    { name: 'includeCritique', type: 'boolean', description: 'Whether to include self-criticism step', default: true },
    { name: 'modelName', type: 'string', description: 'AI model to use', default: 'default' },
    { name: 'temperature', type: 'number', description: 'Model temperature (creativity)', default: 0.7, min: 0, max: 1 }
  ],
  outputs: [
    { name: 'answer', type: 'string', description: 'Final answer or conclusion' },
    { name: 'reasoningPath', type: 'array', description: 'Step-by-step reasoning path' },
    { name: 'confidence', type: 'number', description: 'Confidence in the final answer' },
    { name: 'alternatives', type: 'array', description: 'Alternative reasoning paths or answers' },
    { name: 'error', type: 'string', description: 'Error message if reasoning fails' }
  ],
